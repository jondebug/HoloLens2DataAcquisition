...
public:
	...
	struct BoundingBoxNode
	{
		...
		bool TestRayIntersection(const std::vector<Vertex>& vertices, const XMVECTOR& rayOriginInWorldSpace, const XMVECTOR& rayDirectionInWorldSpace, const XMMATRIX& worldTransform, float &distance, XMVECTOR& normalInLocalSpace, float maxDistance = 999999999 /*= std::numeric_limits<float>::max()*/, bool returnFurthest = false);
	};
	...
	bool TestRayIntersection(const XMVECTOR& rayOriginInWorldSpace, const XMVECTOR& rayDirectionInWorldSpace, const XMMATRIX& worldTransform, float &distance, XMVECTOR &normal, float maxDistance = 99999999 /*= std::numeric_limits<float>::max()*/, bool returnFurthest = false);
